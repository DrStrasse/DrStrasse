...
| 100 | `lua/autorun/sh_grm_jobs.lua` + `lua/entities/grm_jobcenter/` + `lua/entities/grm_depot/` | **Биржа труда v2.1.0 (Код 77):** три работы — **Курьер** (доставка), **Мусоровоз** (маршрут: контейнеры → свалка, нужен транспорт), **Таксист** (посадка → назначение, нужен транспорт). Новый UI: вкладки (Работы / Моя работа / Заказы фракций / Публикация / Доступы), карточки с иконками, наградой и временем. Заказы и вакансии фракций (эскроу с бюджета), `/jobs` `/jobcancel` `/jobpost`, ачивки `GRM_Jobs_Completed` |
