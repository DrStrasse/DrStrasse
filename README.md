...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` | **Пожары v1.0.0 (Код 58):** серверная обвязка аддона `grm_fire` + vFire. Рукава/модели в отдельном аддоне. Права `/fire_access`, оповещение `/grm_fire_notify`, персист очагов, рандом по `grm_fire_spot`, опц. плита. Принтер/пресс не источники |
