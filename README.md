...
биндер: до **40 слотов**, на каждый — название, тип **«в чат»** (уходит как
`say`, работают `/me`, `/dep`, `/fr`) или **«в консоль»**, своя клавиша,
задержка, кулдаун и **связка** с другим слотом через паузу — цепочка
«отыгровка + команда» одним нажатием. Бинды молчат, пока открыт чат, консоль
или меню; цепочки защищены от зацикливания. Настройки хранятся у игрока в
`data/grm_binder.json`.
