...
`env_skypaint`, движение солнца по серверным часам и интерполируемые палитры
рассвета/дня/заката/ночи/шторма. Скомпилированное освещение карты не меняется.
Weather v2.1 использует плавную адаптивную экспозицию: закат темнеет до 21:30,
снаружи ночь выражена сильнее, а внутри на 32% светлее. Холодный lift сохраняет
детали в тенях; общая и личная глубина регулируются без изменения lightmaps.
Также есть плавный world/skybox fog, дождь, гроза, снег и HUD-часы. Weather v2
адаптирует проверенные ресурсы GMod Atmos: объёмные капли, туман осадков,
коллизионные брызги, loop-cued rain и пять вариантов грома. Проверки неба
кешируются по сетке, а под крышей звук плавно приглушается. Оригинальные
По умолчанию музыкальный профиль **«Тёплый живой город»** плавно меняет
воздушный дневной, уютный вечерний и спокойный ночной эмбиенты без тяжёлых
минорных подложек. Альтернативный городской detective-профиль остаётся в
`/weather_admin`, но также переработан в более тёплое и собранное звучание. Аудио использует
Source sound patches; до загрузки пользовательского WAV дождь имеет встроенный
fallback. Физические
