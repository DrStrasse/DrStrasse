...
| 100 | `sh_grm_jobs.lua` + `sh_grm_jobs_config.lua` + `sh_grm_jobs_v4.lua` + `grm_garbage_bin/box` + stool `grm_jobs` | **Биржа труда v4.1:** GRM-инструмент отдельно ставит точки маршрута и физические мусорки модели `trashdumpster01a`; мусорки сохраняются/загружаются отдельной строкой в `/grm_persistence` через единый Perm backend; большое меню назначения Source/simfphys/LVS транспорта каждой работе; физический поиск в мусорке, перенос коробки `cardboard_box03`, загрузка G сзади мусоровоза, вместимость и сдача на свалке; таксист ждёт живые `/calltaxi`/мобильные заказы, принимает клиента по маркеру и получает оплату при посадке. Подробно `CONCEPT_JOBS_V4.md`. |
