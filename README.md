...
| 87 | `lua/autorun/sh_grm_documents.lua` | **Единая система документов GRM Documents & Identity Core v1.4.1:** Паспорта, служебные удостоверения (ксивы), военные билеты, водительские удостоверения (Гражданские ГАИ и Военные ВАИ с категориями A–E+СПЕЦ и 6 войсковыми спецдопусками), документы прикрытия для спецслужб, двухфазный интерактивный рендер (обложка ⇄ разворот), проверка прав при посадке в Т/С, C-меню, полный набор команд `/passport`, `/badge`, `/military`, `/license`, `/millicense`, `/medcard` + `/show*`, настраиваемое название государства и код страны в MRZ (без привязки к реальным странам) |
...
| 59 | `lua/autorun/sh_grm_electronics.lua` + `lua/autorun/client/cl_grm_electronics.lua` + `lua/autorun/client/cl_grm_photorobot.lua` + `lua/autorun/sh_grm_osformat.lua` + `lua/autorun/server/sv_grm_osdb.lua` | **GRM NET OS v2.1.0 + Фоторобот GRMFACE + свои языки и база (Код 59):** сетевая ОС (роутеры, компьютеры `grm_net_computer`, принтеры `grm_net_printer`, файлы/почта/аккаунты, модули фракция/аресты/штрафы/CCTV/прослушка/госуслуги). **Фоторобот — отдельный модуль `GRM.Photorobot`**: свои правила отрисовки (слои-примитивы + роли палитры skin/hair/eye/…, детерминированный шум), свой формат `GRMFACE/1` (текстовый, версионированный, контрольная сумма FNV-1a) → переоткрыть/доредактировать/перепечатать 1:1; захват через off-screen RT (JPEG), универсальная печать, импорт из `data/grm_import/`. **Свои языки `GRM.OSFormat`**: контейнер файла `GRMFILE/1` + язык разметки документов `GRMML` (заголовки/списки/цитаты/`[img:]`/`[grface:]`/инлайн **жирный**/*курсив*) + общая FNV-1a. **Своя база `GRM.OSDB`**: формат `GRMDB/1` — записи МАССИВОМ (без числовых ключей), сигнатура, read-back, backup + карантин битого файла |
