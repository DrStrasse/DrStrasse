...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` + `sh_grm_fire_truck.lua` + `sh_grm_fire_pump_ui.lua` | **Пожары v1.3.4 / аддон 0.4.4 (Код 58):** выдача рукава с машины — G / `/рукав` / E по насосу. Смотка **только на сервере** (`ENT:Think`): проекция на сегмент / S / скорость к предыдущему узлу / ALT. Укладка при ходе назад не кладётся. Рукав на земле — красная лента. Бортовой насос не пермится. Баки 4000/500/250 |
