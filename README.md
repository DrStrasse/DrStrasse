...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` + `sh_grm_fire_truck.lua` + `sh_grm_fire_pump_ui.lua` | **Пожары v1.3.0 (Код 58):** после `/firetruck` клавиша **G** — панель насоса (вода 4000 / пена 500 / порошок 250). Закачка только с открытого гидранта, слив, прямая подача. Тушение списывает бак (~8 л/тик). Лестница `de_train/ladderaluminium` — переносная и на борт |
