...
| 15б | `lua/autorun/sh_grm_fleet.lua` | Автопарк организаций: рынок закупок (суперадмин настраивает класс, цену, уровень допуска, лимит), закупка лидером/ролью с правом `fleet_buy` за счёт бюджета организации, приписка техники к гаражу, выдача сотрудникам ПО МЕСТАМ СТОЯНКИ, возврат и списание (`grm_fleet_scrap`, `grm_fleet_state_share`). Команда `/автопарк` (`/fleet`, `grm_fleet`), вкладка «Автопарк» в терминалах и в окне гаража |
