...
| 58 | `lua/autorun/sh_grm_fire*.lua` + `addons/grm_fire/` | **Пожары v1.5.0 / аддон 0.6.0:** струя без живого vFire больше не создаёт ложный инцидент; одно статусное уведомление на получателя; «локализован» приходит только пока огонь ещё существует, затем «потушен» (быстрый очаг — только «потушен»). Гидрант и пожарное оборудование негорючие и не могут стать родителем очага. Стационарный рукав гидрант↔машина при превышении 2200+96 юнитов 0.75 с автоматически сматывается и отключает прямую подачу. |
