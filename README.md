...
| 72a | `lua/autorun/sh_grm_character.lua` | **Меню выбора персонажа v1.4.0 (Код 72):** большое окно (до 1500×880, адаптивно) с плоской палитрой — левая колонка: имя + 3 крупные карточки слотов (статус/модель), средняя: список внешностей (модель + провайдер, подсветка выбора), правая: крупное 3D-превью + скин/бодигруппы; нижняя панель «Продолжить / Создать и выбрать». RP-имена, слоты (`grm_characters.json`), провайдеры внешности (`GRM.Char.RegisterProvider`), гардероб |
...
| 97 | `lua/entities/grm_comp_fire/` | **Пожарная станция (диспетчерская):** сводка живых очагов и готовности vFire/аддона, дежурство (закрепить/снять машину `/firetruck`, взять ствол/рукав), открытие меню пожарки (доступ/оповещение/машины/очаги/журнал). Доступ: суперадмин, бойцы (`FightPro`), диспетчеры |
...
| 100 | `lua/autorun/sh_grm_jobs.lua` + `lua/entities/grm_jobcenter/` + `lua/entities/grm_depot/` | **Биржа труда v2.2.0 (Код 77):** три работы — **Курьер** (доставка), **Мусоровоз** (маршрут: контейнеры → свалка, нужен транспорт), **Таксист** (посадка → назначение, нужен транспорт). Новый UI: вкладки (Работы / Моя работа / Заказы фракций / Публикация / Доступы / Такси), карточки с иконками, наградой и временем. Заказы и вакансии фракций (эскроу с бюджета), `/jobs` `/jobcancel` `/jobpost`, ачивки `GRM_Jobs_Completed`. **v2.2.0:** типизированные точки/маршруты (`/jobdepot_add [тип]`, `/jobdepot_type`, `/jobdepot_name`: courier/garbage/dump/taxi_pickup/taxi_dest) + **такси-сервис** — водитель задаёт таксу (`/taxifare`, меню `/taxi`), пассажир платит, комиссия в гос.бюджет; суперадмин настраивает ТС-такси, границы таксы и комиссию (`/taxiany`, `/taxiveh_add/del/list`, вкладка «Такси») |
...
| 101 | `lua/autorun/sh_grm_911.lua` + `lua/entities/grm_corpse/` | **Служба 911 v1.0.0 (расследования + трупы + помощь пострадавшим):** летальный урон больше не убивает сразу — игрок **падает без сознания** (Downed, HP=1, ragdoll/freeze, HUD-отсчёт до смерти `bleedout` сек); в Downed добить нельзя. **Оживление** медиком (`/revive`, `GRM.MedicalFull.IsMedic`/`CanTreat` или фракция-медики) за `revive` сек → встаёт с `reviveHp` HP. **Тело** (`grm_corpse`): при смерти остаётся тело с метаданными (имя, время смерти, причина, нападавший, ранения), живёт `corpseTtl`; осмотр по E/`/examine` даёт сводку и пишет запись в журнал происшествий `grm_911_incidents.json`. **Вызов /911** — оповещает онлайн-медиков и полицию с позицией (кулдаун). Суперадмин: `/911cfg`, `/911_set <ключ> <значение>`, `/911_log`; фракции медик/полиция — по названию (`E.FoldCase` без locale). Хуки `GRM_E911_Downed`/`GRM_E911_Revived` |
