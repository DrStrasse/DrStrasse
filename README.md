...
| 43 | `lua/autorun/sh_grm_economy.lua` | **Единая экономика v3.0.1 — ПЕРЕПИСАНО С НУЛЯ (+ патч «банк помнит»):** бюджеты/налоги (только с ЗП!) фракций, зарплаты по ролям/отделам, штрафы `/fine`, история, импорт легаси-данных, админ-панель `/salary_admin` (обновляется без переоткрытия), банк-терминал; команды `!fbudget !fpay !fwithdraw !fpayall !fsettax /mysalary`; канал банка `GRM_Bank_Sync` в Double (без переполнения >4.29 млрд); автосохранение ≤8с, сверка с базой 15с + `/dbcheck`, страж-синглтон. **v3.0.1:** сверка применяет политику валюты «память главнее» — постаревший/воскресший файл не может затереть счета и бюджеты в памяти (отклонение + мгновенное самолечение файла); банковские операции (взнос/снятие/перевод) пишутся на диск СРАЗУ с печатью `SAVE ok: … [причина]`; загрузчик выбирает самый полный источник (свежее зеркало важнее постаревшего основного файла); рентген счетов при загрузке |
