...
| 101 | `lua/autorun/sh_grm_911.lua` | **Система 911 v1.0.0:** летальный урон переводит игрока в тяжёлое состояние с таймером; стабилизация и реанимация через E/`/aid`; вызовы `/911`, диспетчерский список `/911_calls` и маркеры; после смерти остаётся тело с причиной, оружием, нападавшим и следами повреждений; осмотр `/forensics`, опечатывание, доставка в морг и журнал `/911_cases`; единая медкарта получает записи, успешная реанимация субсидирует бюджет медслужбы из казны; `/911_admin`. |
...
| 103 | `lua/autorun/sh_grm_physical_documents.lua` + `grm_item_drop` | **Физические документы v1.0.0:** паспорт, служебное удостоверение, военный билет, гражданские/военные права, лицензии на оружие и бизнес хранятся отдельными предметами инвентаря; до 6 копий каждого документа; автоматическая выдача после оформления и `/doccopy`; использование открывает визуальный бланк владельца; Drop создаёт модель планшета на земле, подбор сохраняет CharacterKey/номер/copyID; аннулирование из реестра видно на всех копиях; оружейная и бизнес-лицензии открываются/предъявляются командами и кнопками C-меню. |
