...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` + `sh_grm_fire_truck.lua` + `sh_grm_fire_pump_ui.lua` + `sh_grm_fire_spots.lua` | **Пожары v1.3.9 / аддон 0.5.0 (Код 58):** точки очага — постановка/удаление/поджог тулом, видны с «Пожарное железо», таймеры `/fire_spots`. G без рейса не орёт инкассацией. Рукава/насос. Смотка S/ALT |
