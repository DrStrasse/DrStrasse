...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` + `sh_grm_fire_truck.lua` + `sh_grm_fire_pump_ui.lua` | **Пожары v1.3.1 (Код 58):** G у машины — **ВЗЯТЬ РУКАВ**, смотка, связь с гидрантом (рядом 380 юн. или прокладка до 2200). E по машине/насосу тоже выдаёт ствол. Рукава красные (`cable/redcable`). Баки вода 4000 / пена 500 / порошок 250 |
