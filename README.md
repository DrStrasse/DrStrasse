...
| 100 | `sh_grm_jobs.lua` + `sh_grm_jobs_config.lua` + `sh_grm_jobs_v4.lua/v5.lua` + `grm_garbage_bin/box` + stool `grm_jobs` | **Биржа труда v5:** GRM-инструмент отдельно ставит точки маршрута и физические мусорки `trashdumpster01a`; live topology связывает их 1:1, исключает несвязанные точки, автоматически обновляет активные маршруты по стабильным ID и ведёт к отдельной свалке; выгрузка читает груз с canonical seat/root, требует остановки в настраиваемой зоне и показывает уведомления/progress-bar; большое автообновляемое окно показывает готовность/cooldown мусорок, связи и груз/фазу машин, водитель видит HUD. Мусорки сохраняются через `/grm_persistence`; физический поиск, коробка `cardboard_box03`, загрузка G сзади и живое такси сохранены. Подробно `CONCEPT_JOBS_V4.md`. |
