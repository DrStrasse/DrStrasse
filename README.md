...
`env_skypaint`, движение солнца по серверным часам, интерполируемые палитры
рассвета/дня/заката/ночи/шторма и сбалансированный LightStyle. Яркость ночи
регулируется в `/weather_admin`, поэтому улицы остаются читаемыми. Также есть
плавный world/skybox fog, мировые частицы дождя, гроза и HUD-часы. Звук дождя
собран из водяного фона, нерегулярных ударов, брызг и близких капель без
постоянного телевизионного шипения; под крышей он приглушается. оригинальные музыкальные эмбиенты плавно меняются для дня, вечера
и ночи. По умолчанию включена отдельная оригинальная тема **Noir Detective** —
медленный контрабасовый пульс, мягкое пианино, щётки и спокойное движение;
в `/weather_admin` можно вернуть профиль по времени суток. Аудио раздаётся через
`resource.AddFile`; после первой установки требуется переподключение. Физические
