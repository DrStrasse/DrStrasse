...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` + `sh_grm_fire_truck.lua` | **Пожары v1.1.0 (Код 58):** обвязка аддона `grm_fire` + vFire. Права `/fire_access`, оповещение `/grm_fire_notify`. Пожарная машина как инкассация: список ТС/ролей в `data/grm_fire/trucks.json`, сел → `/firetruck` вешает насос (4 рукава). Персист очагов, рандом, плита |
