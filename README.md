...
| 96v5 | `lua/autorun/sh_grm_qmenu.lua` | **Большое безопасное Q-меню «Стройка» v5.0.0:** удержание Q; окно до 94%×92% экрана; только каталог пропов, без оружия/NPC/entity/транспорта для игроков; серверные spawn-гейты; безопасный набор строительных инструментов; три колонки «пропы / инструменты / параметры»; ручные схемы без чужого `BuildCPanel`; цветовая палитра `DColorMixer`; личные и серверные настройки; динамический выбор фракции для служебного диспетчера. |
