...
- `expression2/citadel/` — **Ядро Цитадели**: реактор тёмной энергии HL2
  (пульсирующая сфера coreball, выработка/нагрузка/стабильность/коллапс,
  EGP-панель, remote-электросеть) + чип-реле потребителя. См. его README.
- `expression2/rail/` — **Ядро-диспетчер железной дороги**: блок-участки и
  3-значная автоблокировка, светофоры со встроенным датчиком пути,
  звено поезда (радио `rail_grid`), numpad/EGP-пульт, аварийный стоп.
- `expression2/combine_os/` — **CMB-OS**, новая независимая инфраструктура
  Альянса: административный mainframe, до 10 EGP-терминалов с логином/PIN,
  заметками и сообщениями, спавн/опрос датчиков пути, сигнал-узлы и стрелки.
