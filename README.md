...
| 15а | `lua/autorun/sh_grm_plates.lua` + `lua/entities/grm_plate` | Регистрационные номерные знаки: выдача в Полиции/Автоинспекции (вкладка «Номерные знаки» в терминалах), реестр `data/grm_plates/registry.json`, физический знак на `models/hunter/plates/plate025x075.mdl` с материалом `models/debug/debugwhite` — ставится физганом и крепится на транспорт по [E] спереди и сзади. Команды `/номера` (`/plates`, `grm_plates`) и `/номер <А123ВС>` (`/plate`). Конвары `grm_plates_limit`, `grm_plates_blanks`. Автоматическая генерация/крепление номеров за техникой автопарка выключена (заказ владельца 22.08) |
| 15б | `lua/autorun/sh_grm_fleet.lua` | Автопарк организаций (закрепление техники за должностями и отделами): рынок закупок (суперадмин настраивает класс, цену, уровень допуска, лимит), закупка лидером/ролью с правом `fleet_buy` за счёт бюджета организации, приписка техники к гаражу, выдача сотрудникам ПО МЕСТАМ СТОЯНКИ, возврат и списание (`grm_fleet_scrap`, `grm_fleet_state_share`). Убрать в гараж можно и личную машину, и служебную единицу автопарка — в том числе из раздела «На карте» у дилера. Команда `/автопарк` (`/fleet`, `grm_fleet`), вкладка «Автопарк» в терминалах и в окне гаража |
