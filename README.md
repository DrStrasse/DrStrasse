...
| 10 | `lua/autorun/sh_factions.lua` | Ядро фракций: ранги, отделы, приглашения, рация `/fr`, государственная волна `/dep` `/d` (префикс и сообщение принудительно белые, без протекания цвета фракции/EasyChat), OOC `/depb`, меню `/factions`, HUD-таблички |
