...
| 59 | `lua/autorun/sh_grm_electronics.lua` + `lua/autorun/client/cl_grm_electronics.lua` + `lua/autorun/client/cl_grm_photorobot.lua` + `lua/autorun/client/cl_grm_osdoc.lua` + `lua/autorun/sh_grm_osformat.lua` + `lua/autorun/server/sv_grm_osdb.lua` | **GRM NET OS v2.2.0 + Фоторобот GRMFACE + свои языки и база (Код 59):** сетевая ОС (роутеры, компьютеры `grm_net_computer`, принтеры `grm_net_printer`, файлы/почта/аккаунты, модули фракция/аресты/штрафы/CCTV/прослушка/госуслуги). **Фоторобот — модуль `GRM.Photorobot`**: свои правила отрисовки (слои-примитивы + роли палитры, детерминированный шум), формат `GRMFACE/1` (версия + FNV-1a) → переоткрыть/доредактировать/перепечатать 1:1. **Свои языки `GRM.OSFormat`**: контейнер `GRMFILE/1` + язык разметки `GRMML` + общая FNV-1a. **Рендер документов `GRM.OSDoc`**: GRMML-просмотрщик (заголовки/списки/цитаты/`[img:]`/`[grface:]`/инлайн жирный-курсив) в редакторе, файл-менеджере и печатном документе. **Своя база `GRM.OSDB` (формат `GRMDB/1`)**: файлы ОС хранятся МАССИВОМ (без числовых ключей), сигнатура, read-back, backup+карантин; JSON `database.json` остаётся легаси-зеркалом |
