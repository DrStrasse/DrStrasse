...
| 62 | `lua/autorun/sh_01_grm_persistence_guard.lua` | **Persistence Guard v1.1.0:** до загрузки модулей снимает boot-снимок важных DATA-файлов; валидный непустой primary главнее backup, но boot/backup восстанавливает ошибочно опустевший primary. Inventory, Characters, Factions, Documents, Medical, Perm, Electronics, Phone, Factory (включая шкафы/скупщиков/рынок), Food Vending, Logistics, RoomTap, CCTV, Alarm, Services, Diplomas и Vehicle Dealer используют зеркала и fail-closed: ошибка чтения больше не превращается в пустой autosave. Диагностика: серверная команда `grm_persistence_audit`. |
| 79 | `sv_grm_persistence_hub.lua` + `cl_grm_persistence_hub.lua` | **Единое меню сохранений v1.1:** Phone/Food/Factory вызываются через полные `SaveAll/LoadAll`; «Еда и кухня» охватывает автоматы, плиты, холодильники и горшки с содержимым; завод — карту, склады, шкафы, скупщиков и рынок. Отсутствующий модуль теперь даёт красную ошибку, а не ложный SUCCESS; старый id `vending` совместим. |
