...
`GRM.Weather` ведёт общее ускоренное время, утро/день/вечер/ночь, динамический
`env_skypaint`, движение солнца по серверным часам, реальное затемнение карты
ночью через LightStyle и интерполируемые палитры dawn/day/dusk/night/storm,
плавный world/skybox fog, мировые частицы дождя,
грозу, цветокоррекцию и HUD-часы. Осадки имеют фоновые звуки с приглушением
под крышей; оригинальные музыкальные эмбиенты плавно меняются для дня, вечера
и ночи. Физические `grm_clock` ставятся инструментом **GRM Городские часы** и
закрепляются `/permadd`. Панель: `/weather_admin`, просмотр: `/time`.
Описание: [`CONCEPT_PROPERTY_WEATHER.md`](CONCEPT_PROPERTY_WEATHER.md).
