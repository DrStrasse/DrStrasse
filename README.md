...
| 15б | `lua/autorun/sh_grm_fleet.lua` | Автопарк организаций (закрепление техники за должностями и отделами): рынок закупок (суперадмин настраивает класс, цену, уровень допуска, лимит), закупка лидером/ролью с правом `fleet_buy` за счёт бюджета организации, приписка техники к гаражу, выдача сотрудникам ПО МЕСТАМ СТОЯНКИ, возврат и списание (`grm_fleet_scrap`, `grm_fleet_state_share`). Позиция дилера имеет устойчивый `FL.DealerEntryID` (класс+организация+цена+категория), поэтому закупка из окна дилера списывает именно ту цену, что показана на карточке, а не «первую по классу». Убрать в гараж можно и личную машину, и служебную единицу автопарка — в том числе из раздела «На карте» у дилера. Команда `/автопарк` (`/fleet`, `grm_fleet`), вкладка «Автопарк» в терминалах и в окне гаража |
...
| 15к | `lua/autorun/client/cl_grm_vehicle_cells.lua` | `GRM.VehicleCells` — общая ЯЧЕЙКА транспорта (превью, название, номерной знак, состояние, кнопки, меню по ПКМ). Используется у дилера, в окне гаража (личный и служебный) и во вкладке «Автопарк» |
