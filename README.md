...
| 50 | `lua/autorun/sh_grm_perm_entities.lua` + `stools/grm_perm_tool.lua` | **Perm v1.6.1:** `/permadd` и тул используют единый API; UID главнее позиции; занятый объект при `/permload` привязывается к записи и получает данные/владение; сохраняются модель, материал, RGBA, render mode/FX, skin, bodygroups и модульные `rec.data`; временный логистический ящик запрещён. |
