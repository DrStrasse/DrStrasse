...
| 58 | `lua/autorun/sh_grm_fire.lua` + `sh_grm_fire_access.lua` + `sh_grm_fire_truck.lua` + `sh_grm_fire_pump_ui.lua` | **Пожары v1.3.4 (Код 58):** выдача рукава с машины — G / `/рукав` / E по насосу. Рукав на земле — красная лента. Бортовой насос **не пермится** (после рестарта не висит в воздухе); призраки снимает `SweepOrphanGear`. Связь с гидрантом: открыть E + подъехать или «Связать». Баки 4000/500/250 |
