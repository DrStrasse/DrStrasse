...
| 59 | `lua/autorun/sh_grm_electronics.lua` + `lua/autorun/client/cl_grm_electronics.lua` + `lua/autorun/client/cl_grm_photorobot.lua` + `lua/autorun/client/cl_grm_osdoc.lua` + `lua/autorun/client/cl_grm_osfiles.lua` + `lua/autorun/client/cl_grm_oseditor.lua` + `lua/autorun/sh_grm_osformat.lua` + `lua/autorun/server/sv_grm_osdb.lua` | **GRM NET OS v2.3.0 + свои модули, языки и база (Код 59):** сетевая ОС (роутеры, компьютеры `grm_net_computer`, принтеры `grm_net_printer`, файлы/почта/аккаунты, модули фракция/аресты/штрафы/CCTV/прослушка/госуслуги). **Свои модули**: `GRM.Photorobot` (фоторобот: слои-примитивы + роли палитры, детерминированный шум, формат `GRMFACE/1`), `GRM.OSFiles` (файл-менеджер), `GRM.OSEditor` (текстовый редактор, GRMML-шаблоны), `GRM.OSDoc` (рендер документов). **Свои языки `GRM.OSFormat`**: контейнер `GRMFILE/1` + язык разметки `GRMML` (заголовки/списки/цитаты/`[img:]`/`[grface:]`/инлайн жирный-курсив) + общая FNV-1a. **Своя база `GRM.OSDB` (формат `GRMDB/1`)**: файлы ОС хранятся МАССИВОМ (без числовых ключей), сигнатура, read-back, backup+карантин; JSON `database.json` — легаси-зеркало |
