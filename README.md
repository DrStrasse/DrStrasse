...
> **MOST STABLE:** подтверждённая владельцем стабильная кодовая база — `2a351e6`; контрольные хеши и правила отката находятся в [`STABLE.md`](STABLE.md).
> Передача дел новой сессии: **сначала `STABLE.md`**, затем `CHECKPOINT.md` и `HANDOVER.md`.
> Рабочая ветка этой сессии — **`arena/01a0041b-drstrasse`**.
...
## Основные модули (исторические номера Кодов сохранены)
...
| 2 | `lua/autorun/server/sv_grm_logistics.lua` | Сервер логистики v1.3: рейсы матовозок, склады и шкафы. `SaveAll/LoadAll` атомарно учитывает карту, запасы, матрицу доступа, список транспорта и переносимые ящики; JSON валидируется до удаления живых entity, все контуры имеют backup/read-back/fail-closed. |
...
| 5 | `lua/autorun/server/sv_grm_factory_fullcycle.lua` | Сервер завода: крафт, QTE, лом, GPU, скупщик/шкаф оружия. **Persistence v1.1:** единый `SaveAll/LoadAll` сохраняет не только расстановку/склады, но и содержимое оружейных шкафов, запасы скупщиков и цены рынка; все четыре JSON-контура имеют backup, read-back и fail-closed. |
...
| 50 | `lua/autorun/sh_grm_perm_entities.lua` + `stools/grm_perm_tool.lua` | **Perm v1.7.1:** `/permadd`, тул и `/permload` используют единый API; UID — главная идентичность, а legacy-fallback требует ту же модель/тип двери и точку ≤0.25 юн.; соседние и пересекающиеся `prop_physics` (включая рамку+дверь) больше не считаются помехой и восстанавливаются отдельными entity. Загружается весь допустимый JSON без квоты восстановления; boot-проходы дополнены постоянным watchdog 30с. FFD/Sliding получают независимый `rec.door`, закрытую базовую позицию, владельца, видимую solid-коллизию и позднее наложение механизма; visual/material/RGBA/render/skin/bodygroups и модульные `rec.data` сохранены. `SaveAll` обновляет состояние живых UID-записей, а `LoadClasses` позволяет меню еды восстанавливать только кухонные агрегаты. |
...
| 22 | `lua/autorun/server/sv_grm_phone.lua` | Сервер телефонии: звонки, АТС, прослушка и мониторинг. **Persistence v1.1:** `SaveAll/LoadAll`, primary+backup/read-back, валидация массива и классов до удаления живых entity; загрузка карты больше не удаляет телефоны, купленные персонажами через Phone Shop. |
...
| 87 / 60 | `lua/autorun/sh_grm_documents.lua` | **Документы v2.1.0:** паспорта, ксивы, военные билеты, гражданские/военные ВУ. Срок и баллы лицензий задаёт сервер; миграция сразу сохраняется; 12 баллов → 30 дней приостановки с автоматическим восстановлением; срок и баллы отображаются на бланке. Выдача/отзыв разрешены только у открытого служебного терминала в радиусе 260, с rate-limit и фильтрацией данных; `/license_check` закрыта для посторонних. |
...
| 89 | `lua/autorun/sh_grm_medical.lua` | **Медицинская карта v2.0.0:** Категории годности (А, Б, В, Г, Д), группа крови, аллергии, журнал приёмов, показ медкарты игроку `/showmedcard`, интеграция в C-меню и досье обыска |
...
| 91 | `lua/entities/grm_comp_police/` + `lua/autorun/server/sv_grm_comp_terminal.lua` | **Компьютер OrdnungPolizei v1.0.1:** База розыска, штрафы, паспортный стол, ксивы `POL-`. Снятие/объявление розыска идёт через `GRM_CompTerminal_Act` с trusted-путём после `T.CanEdit` (офицер ведомства больше не ловит «Нет прав» + ложный тост «снят») |
...
| 58 | `lua/autorun/sh_grm_fire*.lua` + `addons/grm_fire/` | **Пожары v1.5.1 / аддон 0.6.2:** струя без живого vFire больше не создаёт ложный инцидент; одно статусное уведомление на получателя; «локализован» приходит только пока огонь ещё существует, затем «потушен» (быстрый очаг — только «потушен»). Гидрант и пожарное оборудование негорючие и не могут стать родителем очага. Стационарный рукав гидрант↔машина при превышении 2200+96 юнитов 0.75 с автоматически сматывается и отключает прямую подачу. В 0.6.1: E переключает вентиль, Shift+E выдаёт рукав; шаг укладки 26, повороты сглажены, толщина +0.5, изгибы после стыковки сохраняются. В 0.6.2: строгий расход вода/пена/порошок 8/4/2 за импульс, полный sync насос→машина→HUD ≤0.25с, авт��стоп пустого/полного бака, валидируемая прямая подача. |
| 61 | `lua/autorun/sh_grm_build_tools.lua` + `stools/grm_{camera,light,lamp,material,colour}.lua` | **Кастомные инструменты строительства v1.0.0:** камера, точечный свет, прожектор, материал и цвет с проверкой владения, безопасными пределами/белыми списками, undo/cleanup и поддержкой Perm. Штатные `camera/light/lamp/material/colour/color` жёстко заблокированы даже для суперадмина; Q-меню v4.2 показывает только GRM-варианты. |
| 62 | `lua/autorun/sh_01_grm_persistence_guard.lua` | **Persistence Guard v1.5.0:** до загрузки модулей снимает boot-снимок важных DATA-файлов; валидный непустой primary главнее backup, но boot/backup восстанавливает ошибочно опустевший primary. Inventory, Characters, Factions, Documents, Medical, Perm, Electronics, Phone, Factory (включая шкафы/скупщиков/рынок), Food Vending, Logistics (карта/access/crates), Mining/OreSpawner, RoomTap (map/access/temporary), CCTV, Alarm, Services, Diplomas и Vehicle Dealer используют зеркала и fail-closed: ошибка чтения больше не превращается в пустой autosave. Диагностика: серверная команда `grm_persistence_audit`. |
| 79 | `sv_grm_persistence_hub.lua` + `cl_grm_persistence_hub.lua` | **Единое меню сохранений v1.4:** Phone/Food/Factory/Logistics/Mining/RoomTap/Alarm/CCTV вызываются через полные `SaveAll/LoadAll`; «Еда и кухня» охватывает автоматы, плиты, холодильники и горшки с содержимым; завод — карту, склады, шкафы, скупщиков и рынок; шахта раздельно хранит ручное оборудование и точки автореспавна. Явный клик «Сохранить» считается recovery-транзакцией: прежние blocked primary/backup/boot сначала архивируются в `data/grm_recovery/`, затем текущий живой мир становится авторитетным. Дополнительно пишется `data/grm_equipment/<map>.json` — центральный manifest ожидаемых entity. После старта/cleanup и каждые 30с он создаёт отсутствующее оборудование даже если модульный JSON оказался пустым; повторная сверка идемпотентна по ID/позиции. Итог и число ожидаемых entity идут в чат/консоль; `grm_persistence_status` повторяет отчёт. |
| — | `sv_grm_mining_saver.lua` + `sv_grm_ore_spawner.lua` | **Mining Persistence v1.1:** UID-восстановление скупщиков и ручных узлов без destructive clear; автоматические узлы не попадают в equipment JSON и восстанавливаются только из отдельных spawn points. Оба файла зеркалируются, валидируются и самопроверяются после старта/cleanup каждые 30с. |

## Внешние зависимости

Внутри репозитория уже находятся дилер `sent_vehicle_dealer`, `grm_item_drop`,
RadioNet, Encumbrance, инвентарь и EasyChat. Для полного игрового набора с
настроенными рецептами/транспортом всё ещё используются внешние аддоны:

- ArcCW — оружие и часть заводских рецептов;
- simfphys / LVS / Glide — транспортные реестры и служебные машины;
- ULX / ULib — опциональные административные команды;
- vFire входит в `dist/grm_fire_addon.zip` и ставится вместе с основным GRM;
- `sound/kom_hour.wav` при необходимости добавляется владельцем сервера.
