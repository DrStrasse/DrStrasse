...
| 15в | `lua/weapons/gmod_tool/stools/grm_transport.lua` | ЕДИНЫЙ тул транспорта: зона гаража → места выдачи → стойка → ворота → дилер → связка дилера с гаражом (заменяет разрозненные «GRM: гаражи» и «Точка выдачи транспорта», старые тулы остаются рабочими) |
