...
## Файлы (куски 1–5 + собственные наработки + dop.addons — **156+ lua-файлов** (Код 60 CCTV))
...
| 62 | `lua/autorun/sh_grm_rp_chat.lua` | **RP-чат v1.0.0:** `/me` `/do` `/it` `/try` `/roll` `/w` `/y` `/looc` `/ooc`, локальный чат по радиусу из `sh_grm_chat_config`; совместим с EasyChat |
...
`grm_cctv/<map>.json` (permanent-камеры/мониторы/стойки)
...
`/wanted` `/wanted_access`, `grm_wanted`, `/wanted_set` `/wanted_clear`
