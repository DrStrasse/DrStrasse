...
| 50 | `lua/autorun/sh_grm_perm_entities.lua` | **Пермы разворачиваемых энтити v1.0.0 (Код 50):** банкомат (`grm_bank_terminal`), таксофон, АТС, терминал/proслушка, телефон теперь закрепляются на карте: прицел + `/permadd` `/permremove` `/permlist` в чат или `grm_perm_add/remove/list` в консоль (только суперадмин); переживают рестарт и cleanup (`InitPostEntity`/`PostCleanupMap`), заморожены; база-массив `data/grm_perm_entities.json` (без числовых ключей — урок находки 65), дедуп 6 юнитов, лимит 64/карту, карантин при битом файле |
