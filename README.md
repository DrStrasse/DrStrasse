...
| 74 | `lua/autorun/sh_grm_f4menu.lua` | **F4-меню v1.2.0:** вкладки Профиль / Команды / Настройки (HUD-выключатели) / **Графика** (пресеты FPS+/Красота, 9 тоглов, слайдеры дальности отрисовки, LOD, текстур, декалей, FPS). F4 ловится биндом И прямым опросом клавиши (антидубль, toggle открыть/закрыть), уступает дверям; `/menu` `/f4` |
