...
| 10 | `lua/autorun/sh_factions.lua` | Ядро фракций: ранги, отделы, приглашения, `/fr`, белая госволна `/dep` `/d`, OOC `/depb`, меню `/factions`; callback-формы ролей/отделов/создания/переименования очищают поля до `refreshAllUI` и проверяют `IsValid`, поэтому ответы сервера не вызывают `Tried to use a NULL Panel`. |
