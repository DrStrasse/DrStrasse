...
| 59 | `lua/autorun/sh_grm_electronics.lua` + `lua/autorun/client/cl_grm_electronics.lua` + `lua/autorun/client/cl_grm_photorobot.lua` | **GRM NET OS v2.1.0 + Фоторобот GRMFACE (Код 59):** сетевая ОС (роутеры, компьютеры `grm_net_computer`, принтеры `grm_net_printer`, файлы/почта/аккаунты, модули фракция/аресты/штрафы/CCTV/прослушка/госуслуги). **Фоторобот переписан в отдельный модуль `GRM.Photorobot`** — свои правила отрисовки (слои-примитивы + роли палитры skin/hair/eye/…, детерминированный шум), свой формат файла `GRMFACE/1` (текстовый, версионированный, контрольная сумма FNV-1a) → фоторобот можно переоткрыть/доредактировать/перепечатать 1:1; захват через off-screen RT (JPEG), универсальная печать, импорт из `data/grm_import/`; `op photorobot_save` хранит `grm`+`desc`+`imagePath` |
