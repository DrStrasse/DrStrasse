...
| 15е | `lua/autorun/sh_grm_prop_guard.lua` | Пропы: новый проп появляется ПРИЗРАКОМ (полупрозрачный, без коллизии и физики) и становится твёрдым после заморозки физганом; снял с заморозки — снова призрак. Защита от спама: очередь пропов закрывает спавн на минуту (`grm_prop_spam_count/window/block`, `grm_prop_spam_admins`), команды `grm_prop_status`, `grm_prop_unblock` |
