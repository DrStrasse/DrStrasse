...
## Где что лежит
- `lua/` — весь код (57 модулей; см. таблицу в README).
- Ключевые: Код 42 `lua/autorun/sh_grm_currency.lua` (валюта **v2.0.2**),
  ...
  Код 51 `lua/autorun/sh_grm_incassation.lua` (инкассация **v2.0.0** — переписано с нуля).
...
- `tools/luatest/roundtrip_test.lua` — мок GMod API + 15 фаз юнит-теста
  персистентности (сохранение данные/денег/пермов/штрафов/инкассации). Обязателен к прогону
...
- HUD селектор (Код 48, v10.3): пока физган/гравиган держит проп (`IN_ATTACK`),
  колесо/слоты/`lastinv` не открывают бар и не зовут `input.SelectWeapon`
  (иначе проп падал при вращении). Не путать с HOLD-Q.
- Пожары v1.3.4 / аддон 0.4.4: бортовой насос не пермится (после рестарта не висит в воздухе).
  SweepOrphanGear на boot. Выдача: G / `/рукав`. Рукав на земле — DrawBox.
  Смотка **серверная** (`HoseMoveHint` + `ENT:Think`): назад по линии / S / ALT.
  Нужны оба zip (`grm_single_addon.zip` и `grm_fire_addon.zip`).
