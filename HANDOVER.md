...
- Пожары v1.4.0 / аддон 0.5.0: учёт тушения — «Пожар локализован»
  (очаг сжат стволом, больше не растёт) и «Пожар потушен» (клеток нет).
  Журнал `data/grm_fire/log.json`. Точки очага с тулом. Нужен
  `grm_single_addon.zip` + рестарт.
