...
(находки 1–70 — там вся история разработки и уроки, НЕ повторяй старые ошибки).

## Где что лежит
- `lua/` — весь код (основные модули ядра + интегрированный `grm_single_addon (4)` + переработанная система дверей v2.0.0 + дверные ключи `ds_key_swep`).
- Ключевые:
  ...
  - Коды 61–62 (`sh_grm_doors.lua` и `sh_grm_doors_access.lua` — система дверей **v2.0.0** & Access **v2.1.0** с ролевым контролем ордеров по Фракциям/Рангам/Отделам в `/factions`),
...
- Следующий свободный номер модуля: **Код 67**.
...
## Текущее состояние
- Созданы дверные ключи `ds_key_swep` (ЛКМ — закрыть, ПКМ — открыть, R — меню двери).
- Доступ к ордерам на обыск (/warrant) полностью интегрирован в меню `/factions` с гибким разграничением по Фракциям, Рангам, Подразделениям/Отделам и SteamID.
- Тесты пройдены 13/13 (100% OK), синтаксис всех 473 Lua-файлов репозитория 100% валиден.
