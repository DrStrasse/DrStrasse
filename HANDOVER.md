...
3D-вьюпорт с гизмо, снимок кадра, конструктор окон «МАКЕТ» (виджеты,
тест окна, шаблоны), «ПРОВЕРКА» (валидатор + синтаксис), компиляция в
черновик GLua + тул размещения (undo). Файлы: `sh_/sv_/cl_grm_addon_studio.lua`,
стенд `sim_addon_studio.lua` (72 проверки). Генератор выдаёт текст,
который владелец копирует и присылает агенту — точечная реализация по
манифесту, а не догадки.
