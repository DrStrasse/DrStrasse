...
(находки 1–68 — там вся история разработки и уроки, НЕ повторяй старые ошибки).

## Где что лежит
- `lua/` — весь код (основные модули ядра + интегрированный `grm_single_addon (4)` + дополнительные аддоны).
- Ключевые:
...
## Текущее состояние
- Все новые подсистемы из `grm_single_addon (4).zip` (CCTV, Alarms, Doors, Wanted, RP-chat, Item Drop) извлечены, спаяны с ядром, протестированы и включены в релизные архивы.
