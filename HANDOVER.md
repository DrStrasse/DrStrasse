...
- Пожары v1.3.7 / аддон 0.4.9: G у насоса/пожарной машины — только панель
  насоса (`F.IsFireGContext`). Инкассация на этом G молчит (нет тоста
  «нет рейса»). Смотришь прямо на банкомат/вольт — инкассация как была.
  ТС удалили — рукава/насос сразу. Наземный насос из тула не трогаем.
  Нужны оба zip: `grm_single_addon.zip` + `grm_fire_addon.zip`.
