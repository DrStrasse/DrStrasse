...
- Пожары v1.3.9 / аддон 0.5.0: точки очага видны только с тулом
  «GRM Пожарное железо». ЛКМ поставить/настроить, ПКМ поджечь, R снять.
  Таймеры `/fire_spots` (data/grm_fire/config.json). Нужны оба zip.
