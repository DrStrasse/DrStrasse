...
**Сначала читай `CHECKPOINT.md`** — полная контрольная точка Кода 58
(пожары): HEAD, незакрытый заказ «локализован/потушен», журнал всех
ошибок и граблей, что трогать / не трогать, чеклист хода.

Если ты — новая сессия Arena-агента в этом репозитории: **не работай от master** —
он почти пустой (снапшот пра-истории). Вся актуальная база **этой сессии** в ветке
`arena/019ffaa2-drstrasse` (не в `019f69c8`):

```
git fetch origin arena/019ffaa2-drstrasse
git reset --mixed FETCH_HEAD   # mixed, не hard — дерево сохранится
```

После CHECKPOINT прочитай `README.md` (таблица модулей) и `ANALYSIS.md`
(находки 1–125 — история боли, НЕ повторяй старые ошибки).
...
- Пожары v1.4.0 / аддон 0.5.0: учёт тушения лежит в `sh_grm_fire_status.lua`
  (коммит `3430a00`). Владелец **повторил** заказ «локализован/потушен» —
  условия слишком строгие, UI журнала нет. План v1.4.1 в `CHECKPOINT.md` §2.
  Нужны оба zip + рестарт. Не звать `SweepOrphanGear` на одну ТС.
