...
(находки 1–99 — там вся история разработки и уроки, НЕ повторяй старые ошибки).
...
- Следующий свободный номер модуля: **Код 85**.
...
- Изолированные сим-стенды: `tools/luatest/sim_broadcast.lua`, `tools/luatest/sim_jobs.lua`, `tools/luatest/sim_trunk.lua`, `tools/luatest/sim_money.lua`, `tools/luatest/sim_qmenu.lua`, `tools/luatest/sim_dealer.lua` (реальный dealer init.lua грузится трансформацией `continue`→goto; стабы ConVar/include/ENT-метатаблица), `tools/luatest/sim_rootboard.lua` (Root Guard + доска-assign + qmenu КЛИЕНТ-пассом: файл грузится дважды со сменой SERVER/CLIENT и LocalPlayer-стабом) — паттерн доказательства чат-контракта и жизненных циклов без GMod.
- Тесты пройдены 13/13 (100% OK), синтаксис всех 226 Lua-файлов ядра 100% валиден, 7 сим-стендов зелёные, proto_audit чист.
