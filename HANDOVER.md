...
(находки 1–93 — там вся история разработки и уроки, НЕ повторяй старые ошибки).
...
- Следующий свободный номер модуля: **Код 84**.
...
- Изолированные сим-стенды: `tools/luatest/sim_broadcast.lua`, `tools/luatest/sim_jobs.lua`, `tools/luatest/sim_trunk.lua`, `tools/luatest/sim_money.lua`, `tools/luatest/sim_qmenu.lua`, `tools/luatest/sim_dealer.lua` (реальный dealer init.lua грузится трансформацией `continue`→goto; стабы ConVar/include/ENT-метатаблица) — паттерн доказательства чат-контракта и жизненных циклов без GMod.
- Тесты пройдены 13/13 (100% OK), синтаксис всех 225 Lua-файлов ядра 100% валиден.
