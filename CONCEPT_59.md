...
**Следующие шаги (порядок):** 1) редактор/файл-менеджер/галерея как свои
модули (вынести из монолита); 2) оконный менеджер; 3) единая тема и правила
доступа (rate-limit / OS Type / роли) в общий модуль; 4) связь фоторобота с
документами (фото-аватар) и розыском (фото-архив полиции).
