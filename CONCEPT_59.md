...
## 0a. Курс «своё» — языки / модули / правила / база

Владелец закрепил принцип: фоторобот и компьютер (GRM NET OS) делаем ПОЛНОСТЬЮ
своим — свои языки, свои модули, свои правила, своя база. Карта соответствия:

| Что | Своя реализация | Статус |
|---|---|---|
| Язык/формат фоторобота | `GRMFACE/1` (текст, версия, FNV-1a) | ✅ находка 129 |
| Язык контейнера файла ОС | `GRMFILE/1` (`GRM.OSFormat.FileEncode/Decode`) | ✅ находка 130 |
| Язык разметки документов ОС | `GRMML` (`GRM.OSFormat.Markup.Parse/Inline`) — вместо HTML/DHTML | ✅ находка 130 |
| Общая контрольная сумма | FNV-1a без `util.CRC` (`GRM.OSFormat.Checksum`) | ✅ находка 130 |
| Своя база ОС | `GRMDB/1` (`GRM.OSDB`) — записи массивом, сигнатура, read-back, backup+карантин | ✅ находка 130 |
| Свои правила отрисовки | слои-примитивы + роли палитры (фоторобот); GRMML-рендер (OSDoc) | ✅ фото + ✅ документы |
| Свои модули | `GRM.Photorobot`; `GRM.OSFormat`; `GRM.OSDB`; `GRM.OSDoc` | ✅ |
| Рендер документов своим языком | `GRM.OSDoc` (GRMML) в редакторе/файл-менеджере/печатном документе | ✅ находка 131 |
| Своя база для файлов OS | `E.Files` пишется в `GRM.OSDB` (стор `netfiles`, GRMDB/1); JSON — легаси-зеркало | ✅ находка 131 |
| Приложения OS как модули | `GRM.OSFiles` (файл-менеджер), `GRM.OSEditor` (редактор), галерея в `GRM.Photorobot` | ✅ находка 132 |
| Свои правила доступа | rate-limit / OS Type / роль-палитры — систематизировать в общий модуль | 🔜 |
| Оконный менеджер | drag/minimize/трей | 🔜 |

**Следующие шаги (порядок):** 1) оконный менеджер (drag/minimize/трей);
2) единая тема и правила доступа (rate-limit / OS Type / роли) в общий
модуль `GRM.OSAccess`; 3) связь фоторобота с документами (фото-аватар) и
розыском (фото-архив полиции).
