...
| Свои правила отрисовки | слои-примитивы + роли палитры (фоторобот); дальше — единая тема/рендер OS | ✅ фото / 🔜 OS |
| Свои модули | `GRM.Photorobot`; `GRM.OSFormat`; `GRM.OSDB` | ✅ частично |
| Приложения OS как модули | редактор/файл-менеджер/галерея — вынести из монолита `cl_grm_electronics.lua` | 🔜 |
| Своя база для файлов OS | миграция `E.Files`/`E.SaveDB` (JSON per-device) на `GRM.OSDB` | 🔜 |
| Свои правила доступа | rate-limit / OS Type / роль-палитры — систематизировать в общий модуль | 🔜 |

**Следующие шаги (порядок):** 1) мигрировать файлы ОС на `GRMDB`; 2) рендер
документов через `GRMML` + редактор как модуль; 3) файл-менеджер/оконный
менеджер как свои модули; 4) единая тема и правила доступа.
