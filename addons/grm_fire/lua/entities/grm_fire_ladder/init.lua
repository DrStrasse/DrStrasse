...
function ENT:Initialize()
    ...
    self:SetModel(A and A.SafeModel(A.Models.ladder) or "models/props_c17/metalladder002.mdl")
...
function ENT:Use(ply)
    ...
    if IsValid(self:GetHostVehicle()) then
        ...
        if ply.ChatPrint then
            ply:ChatPrint(on and "[Лестница] Выдвинута. Подойдите и идите вперёд — залезете."
                or "[Лестница] Убрана на борт.")
        end
        return
    end
    if ply.ChatPrint then
        ply:ChatPrint("[Лестница] Подойдите вплотную и идите вперёд / прыжок — залезть, назад — спуститься.")
    end
end
