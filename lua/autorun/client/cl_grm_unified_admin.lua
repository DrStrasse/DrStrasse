...
local function open()
 local f=vgui.Create("DFrame"); f:SetTitle("GRM — ЕДИНЫЙ ЦЕНТР УПРАВЛЕНИЯ"); f:SetSize(math.min(760,ScrW()-60),math.min(520,ScrH()-60)); f:Center(); f:MakePopup()
 ...
 local sync=vgui.Create("DButton",f); sync:SetPos(25,350); sync:SetSize(685,45); sync:SetText("СИНХРОНИЗИРОВАТЬ СОСТОЯНИЕ GRM"); sync.DoClick=function() RunConsoleCommand("grm_persistence_admin"); chat.AddText(Color(80,210,150),"[GRM] Запрошена синхронизация сохранений") end
end
