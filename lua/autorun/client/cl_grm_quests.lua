...
local function openNPC()
 ...
 local function show(row)
  ...
  if#nodes>0 then button(detail,"▶  Начать диалог ("..#nodes.." реплик)",18,382,376,42,C.violet or C.blue,function()playDialogue(npcName,nodes,function(action)if action=="accept"then if not p and row.available then playerOp("accept",d.id);f:Close()elseif p and p.status=="completed"and d.repeatable then playerOp("restart",d.id);f:Close()end end end)end)end
...
hook.Add("HUDPaint","GRM_Quest_CutsceneHUD",function()local s=Q.Cutscene;if not s.active then return end;local n=s.nodes[s.index]or {};draw.RoundedBox(0,0,0,ScrW(),62,Color(0,0,0,235));draw.RoundedBox(0,0,ScrH()-82,ScrW(),82,Color(0,0,0,235));if n.image and n.image~=""then surface.SetMaterial(Material(n.image,"smooth"));surface.SetDrawColor(255,255,255,220);surface.DrawTexturedRect(ScrW()/2-80,70,160,90)end;draw.SimpleText(n.caption or "","GRMQ_Head",ScrW()/2,ScrH()-42,C.text,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER);draw.SimpleText("ПРОБЕЛ — пропустить","GRMQ_Small",ScrW()-18,ScrH()-18,C.dim,TEXT_ALIGN_RIGHT,TEXT_ALIGN_CENTER)end)
...
local function adminStudio(data)
 ...
 local choiceList=darkList(dialogues,296,265,548,150,{{"Ответ игрока",300},{"Следующий ID",120},{"Действие",100}});local chText=textEntry(dialogues,"Текст ответа",296,430,260);local chNext=textEntry(dialogues,"Следующий ID",568,430,132);local chAction=comboEntry(dialogues,"Действие",712,430,132,{{"Продолжить",""},{"Принять квест","accept"},{"Закрыть","close"}})
 ...
 local function rebuildChoices()choiceList:Clear();for i,ch in ipairs(currentChoices)do local line=addDarkLine(choiceList,ch.text,ch.next,ch.action);line._index=i end end
 ...
 dlgPhase.OnSelect=function(_,_,_,data)dlgPhase.ValueID=data;selectedNode=0;rebuildNodes()end;dlgList.OnRowSelected=function(_,_,line)loadNode(line._index)end;choiceList.OnRowSelected=function(_,_,line)local ch=currentChoices[line._index];if ch then selectedChoice=line._index;chText:SetText(ch.text or"");chNext:SetText(ch.next or"");chAction.ValueID=ch.action or"";chAction:SetValue(ch.action=="accept"and"Принять квест"or(ch.action=="close"and"Закрыть"or"Продолжить"))end end
 ...
 button(dialogues,"+ Ответ",296,488,100,34,C.blue,function()currentChoices[#currentChoices+1]={text=chText:GetText(),next=chNext:GetText(),action=chAction.ValueID or""};rebuildChoices()end);button(dialogues,"Изменить",404,488,100,34,C.green,function()if selectedChoice>0 then currentChoices[selectedChoice]={text=chText:GetText(),next=chNext:GetText(),action=chAction.ValueID or""};rebuildChoices()end end);button(dialogues,"Удалить",512,488,90,34,C.red,function()if selectedChoice>0 then table.remove(currentChoices,selectedChoice);selectedChoice=0;rebuildChoices()end end)
