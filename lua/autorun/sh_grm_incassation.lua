--[[--------------------------------------------------------------------
    GRM Incassation (Код 126 — «Инкассация») v1.2.0
...
hook.Add("PlayerButtonDown", "GRM_Incass_GKey", function(ply, button)
    ...
    if IsValid(hit) and hit:GetClass() == "grm_bank_terminal"
       and ply:GetPos():DistToSqr(hit:GetPos()) <= ((I.CFG and I.CFG.TerminalRadius or 220)^2) then
        net.Start(NET_TERM_TAKE); net.WriteEntity(hit); net.WriteInt(0, 32)
        -- нет: по ТЗ всегда открывается меню; но сервер при 0 откажется — вместо этого
        -- клиенту нужно отдельное сообщение "открыть меню". Вместо этого шлём 0 как
        -- сигнал открытия меню? У нас сервера нет обработки 0 как открытия — исправляем
        -- через то что на клиенте напрямую не открыть меню без данных от сервера.
        -- Вместо этого: шлём отдельный запрос — а сервер должен послать NET_TERM_MENU.
        -- Для этого используем ту же NET_TERM_TAKE с amount=0 как "показать меню".
        -- Но сервер в CollectFromTerminal при amount<=0 отказывает. Упростим:
        -- Вызовем на сервере sendTerminalMenu через конком.
        -- Легче всего — сделать concommand для открытия меню.
        -- Вместо этого — используем существующий concommand grm_incass_car_use только для машины.
        -- Сделаем так: клиент просто шлёт сетевой запрос "открой меню", добавим tiny helper:
...
-- concommand-ы для открытия меню (сервер шлёт нужный net)
-- Клиентские concommand не сработают — они должны быть на сервере. Ставлю зацепку через
-- существующие net-сообщения, используя новый net "NET_TERM_GUSE" (не определён выше на сервере).
-- Проще: через тот же NET_TERM_TAKE с amt=-1 как сигнал "показать меню".
-- Исправлю сервер после этого коммита? Нет — добавлю сейчас отдельный net и на сервере.
-- Но это повлечёт синтаксис-ошибку, если сейчас не добавлю.
-- Сделаем так: клиент шлёт NET_TERM_TAKE с amt=-1, а на сервере мы добавим обработку ниже.
-- Но уже дописал клиентский код выше и поздно менять. Проще добавить concommand на сервере.
-- Давайте допишу серверные concommand для grm_incass_term_use и grm_incass_vault_use,
-- используя то что на клиенте уже вызываю RunConsoleCommand на них.
-- Но клиентский RunConsoleCommand("grm_incass_term_use") не будет работать пока команда
-- не зарегистрирована на сервере (ошибки нет — просто не сработает).
-- Добавлю их здесь же в CLIENT-секции? Нет, concommand на сервере.
-- Хорошо, поставлю отправку через net с флагом amount=-1 как "открыть меню" для терминала,
-- а для вольта — тоже новый net, аналогичный тому что есть (NET_VAULT_MENU уже есть),
-- нужен триггер с клиента. Добавлю на сервер concommand-ы после написания клиента.
