...
if SERVER then
 ...
 local function inform(rec,text)if not rec or tostring(rec.serviceID or"")==""then return end;local S=GRM.Services;for _,p in ipairs(player.GetAll())do if IsValid(p)and S and S.FactionOf and S.FactionOf(p)==rec.faction then if GRM.Notify then GRM.Notify(p,text,105,210,145)else p:ChatPrint("[Услуги] "..text)end end end end
