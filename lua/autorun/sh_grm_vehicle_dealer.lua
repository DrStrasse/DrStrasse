...
local function write(path,data)if VD.LoadBlocked[path]then local g=GRM.PersistenceGuard;if g and g.AllowBlockedWrite and g.AllowBlockedWrite(path,path..".backup","vehicle dealer")then VD.LoadBlocked[path]=nil else print("[GRM VehicleDealer][!] SAVE ОТКЛОНЁН "..path)return false end end;local guard=GRM.PersistenceGuard;if guard and guard.WriteMirrored then return guard.WriteMirrored(path,path..".backup",data,"vehicle dealer")end;local ok,s=pcall(util.TableToJSON,data,true);if not ok or not isstring(s)then return false end;file.Write(path,s);file.Write(path..".backup",s);return file.Read(path,"DATA")==s end
...
if SERVER then
 ...
 function VD.Push(ply,dealer)local g=garage(ply);local garageRows={}for _,r in pairs(g)do garageRows[#garageRows+1]=r end;local catalog={}for _,e in ipairs(dealer.VD_Vehicles or{})do if VD.CanUseEntry(ply,e)then local i=VD.VehicleInfo(e.class);catalog[#catalog+1]={class=e.class,name=e.name or i.name,model=i.model,system=i.system,price=math.max(0,math.floor(tonumber(e.price)or 0)),category=e.category or"Транспорт",service=e.service==true,faction=e.faction}end end;net.Start("GRM_VD_Open")net.WriteEntity(dealer)net.WriteString(dealer:GetDealerName())net.WriteTable(catalog)net.WriteTable(garageRows)net.Send(ply)end
 ...
 net.Receive("GRM_VD_Action",function(_,ply)local dealer,op=net.ReadEntity(),net.ReadString();if not IsValid(dealer)or dealer:GetClass()~="sent_vehicle_dealer"or ply:GetPos():DistToSqr(dealer:GetPos())>300*300 then return end;ply.GRMVDNext=ply.GRMVDNext or 0;if CurTime()<ply.GRMVDNext then return end;ply.GRMVDNext=CurTime()+.35
  if op=="buy"then local class=net.ReadString();local entry=findEntry(dealer,class);if not entry or not VD.CanUseEntry(ply,entry)then result(ply,false,"Транспорт недоступен")return end;if activeCount(ply)>=VD.MaxActive then result(ply,false,"Лимит активного транспорта")return end;local price=entry.service and 0 or math.max(0,math.floor(tonumber(entry.price)or 0));if price>0 and(not GRM.HasMoney or not GRM.HasMoney(ply,price))then result(ply,false,"Недостаточно средств")return end;local ent,info,spawnErrors=VD.Spawn(class,dealer,ply);if not ent then result(ply,false,(spawnErrors and spawnErrors[1])or"Не удалось создать транспорт")return end;if price>0 and GRM.TakeMoney then GRM.TakeMoney(ply,price,"Покупка транспорта "..class)end;local id=makeID("vehicle");local record={id=id,class=class,name=entry.name or info.name,model=info.model,price=price,stored=false,dealerID=dealer:GetDealerID(),service=entry.service==true};if not entry.service then local g=garage(ply);g[id]=record;saveGarage()end;ent.GRMGarageID=id;ent.GRMGarageOwner=ply;ent.VD_Owner=ply;ent.VD_Class=class;ent.VD_Price=price;VD.Active[id]=ent;hook.Run("GRM_VehicleDealerSpawned",ent,ply,class,record,dealer);if GRM.VehicleKeys and GRM.VehicleKeys.SetPlayerOwner then pcall(GRM.VehicleKeys.SetPlayerOwner,ent,ply)elseif VK and VK.SetPlayerOwner then pcall(VK.SetPlayerOwner,ent,ply)end;result(ply,true,"Транспорт выдан: "..record.name);VD.Push(ply,dealer)
  elseif op=="retrieve"then local id=net.ReadString();local g=garage(ply);local r=g[id];if not r or r.service then result(ply,false,"Запись гаража не найдена")return end;if IsValid(VD.Active[id])then result(ply,false,"Транспорт уже выдан")return end;if activeCount(ply)>=VD.MaxActive then result(ply,false,"Лимит активного транспорта")return end;local ent,_,spawnErrors=VD.Spawn(r.class,dealer,ply);if not ent then result(ply,false,(spawnErrors and spawnErrors[1])or"Не удалось выдать транспорт")return end;r.stored=false;saveGarage();ent.GRMGarageID=id;ent.GRMGarageOwner=ply;ent.VD_Owner=ply;ent.VD_Class=r.class;ent.VD_Price=r.price;VD.Active[id]=ent;result(ply,true,"Транспорт выдан из гаража");VD.Push(ply,dealer)
 ...
 hook.Add("InitPostEntity","GRM_VD_Load",function()timer.Simple(1.5,function()migrateLegacyDealers();VD.LoadDealers()end)end);hook.Add("PostCleanupMap","GRM_VD_Cleanup",function()timer.Simple(.8,VD.LoadDealers)end)
