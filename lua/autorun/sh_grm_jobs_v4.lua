...
local function online(characterKey)for _,p in ipairs(player.GetAll())do if key(p)==characterKey then return p end end end
...
if SERVER then
 ...
 local function taxiDrivers()local out={};for _,p in ipairs(player.GetAll())do local j=active(p);if istable(j)and j.tplId=="taxi"and j.taxiStandby then out[#out+1]=p end end;return out end
 ...
 function JB.SearchGarbageBin(ply,bin)
  ...
  local gv=Vector(goal.x or 0,goal.y or 0,goal.z or 0);if bin:GetPos():DistToSqr(gv)>280*280 then notify(ply,"Это не текущая мусорка маршрута.",false)return end;if bin:GetReadyAt()>CurTime()then notify(ply,"Мусорку уже обыскали. Подождите.",false)return end;if(ply._grmGarbageSearch or 0)>CurTime()then return end
  local duration=tonumber(JB.WorkConfig and JB.WorkConfig.garbageSearchTime)or 2.5;ply._grmGarbageSearch=CurTime()+duration;ply:SetNWBool("GRM_SearchingGarbage",true);notify(ply,"Ищем подходящие отходы...",true)
  timer.Simple(duration,function()if not IsValid(ply)then return end;ply:SetNWBool("GRM_SearchingGarbage",false);if not IsValid(bin)or ply:GetPos():DistToSqr(bin:GetPos())>180*180 or IsValid(ply:GetNWEntity("GRM_GarbageBox"))then return end;local curJob=active(ply);if not(istable(curJob)and curJob.tplId=="garbage")then return end;local current,currentIdx=currentGarbageGoal(curJob);if currentIdx~=idx then return end;local box=ents.Create("grm_garbage_box");if not IsValid(box)then return end;box:SetPos(ply:GetPos());box:SetSourcePointID(tostring(idx));box:Spawn();box:Activate();box:AttachTo(ply);bin:SetReadyAt(CurTime()+(tonumber(JB.WorkConfig.garbageBinCooldown)or 90));notify(ply,"Коробка собрана. Поднесите её сзади к мусоровозу и нажмите G.",true)end)
 end
 ...
 local function nearestGarbageVehicle(ply)local best,dist=nil,230*230;for _,e in ipairs(ents.GetAll())do if IsValid(e)and(e:IsVehicle()or e.GetDriver)then local d=ply:GetPos():DistToSqr(e:GetPos());if d<dist and JB.IsVehicleClassAllowed(e,"garbage")then local behind=(ply:GetPos()-e:GetPos()):Dot(e:GetForward())<0;if behind then best,dist=e,d end end end end;return best end
 local function loadGarbage(ply)
  ...
  local cap=tonumber(JB.WorkConfig and JB.WorkConfig.garbageCapacity)or 8;local load=tonumber(veh.GRM_GarbageLoad)or 0;if load>=cap then notify(ply,"Мусоровоз заполнен.",false)return end;veh.GRM_GarbageLoad=load+1;veh:SetNWInt("GRM_GarbageLoad",load+1);box:Remove();j.garbageCollected=(tonumber(j.garbageCollected)or 0)+1;j.pointIndex=(tonumber(j.pointIndex)or 1)+1;saveJobs("garbage loaded");JB.PushTracker(ply);JB.PushMyState(ply);local nextName=(j.pointNames or{})[j.pointIndex]or"Свалка";audit("garbage.load",ply,{vehicle=veh:EntIndex()},{load=load+1,capacity=cap});notify(ply,"Коробка загружена: "..(load+1).."/"..cap..". Дальше: "..nextName,true)
 end
 net.Receive(NGARBAGE,function(bits,ply)if not IsValid(ply)then return end;if GRM.Net and not GRM.Net.Guard(ply,"jobs.garbage.load",{rate=.5,burst=2,maxBits=256},{bits=bits})then return end;loadGarbage(ply)end)
