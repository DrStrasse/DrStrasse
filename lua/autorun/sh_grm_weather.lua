...
-- Perceptual daylight is deliberately wider than the geometric sun arc:
-- dusk remains readable after sunset and reaches full night only near 21:30.
function W.AmbientFactor(minutes)
 local h=((tonumber(minutes)or W.TimeMinutes())/60)%24
 if h>=8 and h<17.5 then return 1 end
 if h>=5 and h<8 then local t=(h-5)/3;return t*t*(3-2*t)end
 if h>=17.5 and h<21.5 then local t=(h-17.5)/4;return 1-(t*t*(3-2*t))end
 return 0
end
function W.ExposureTarget(ambient,depth,isOutside,weather,personal)
 local night=1-math.Clamp(tonumber(ambient)or 1,0,1);depth=math.Clamp(tonumber(depth)or.5,.2,.85);personal=math.Clamp(tonumber(personal)or 1,.5,1.25)
 local shelter=isOutside and 1 or .68;local weatherDim=weather=="storm"and .018 or weather=="rain"and .009 or weather=="cloudy"and .004 or 0
 return night*Lerp(depth,.055,.125)*shelter*personal+weatherDim,night*Lerp(depth,.16,.34)*personal+(weather=="storm"and .06 or 0)
end
if SERVER then
 ...
 local function sync()SetGlobalFloat("GRM_TimeMinutes",W._time);SetGlobalString("GRM_Weather",W._weather);SetGlobalFloat("GRM_SunFactor",W.SunFactor(W._time));SetGlobalFloat("GRM_AmbientFactor",W.AmbientFactor(W._time));SetGlobalBool("GRM_WeatherHUD",W.Config.hudClock~=false);SetGlobalFloat("GRM_WeatherSoundVolume",math.Clamp(tonumber(W.Config.soundVolume)or.48,0,1));SetGlobalFloat("GRM_WeatherMusicVolume",math.Clamp(tonumber(W.Config.musicVolume)or.18,0,1));SetGlobalBool("GRM_WeatherMusic",W.Config.musicEnabled~=false);SetGlobalString("GRM_WeatherMusicTheme",W.Config.musicTheme=="noir"and"noir"or"periods");SetGlobalFloat("GRM_NightDarkness",math.Clamp(tonumber(W.Config.nightDarkness)or.5,.2,.85))end
...
if CLIENT then
    ...
    local cvNight = CreateClientConVar("grm_weather_night_strength", "1", true, false, "Personal night tone strength")
    local smoothFog, outside = 0, false
    local exposureNow,colorNow=0,0
    ...
    hook.Add("RenderScreenspaceEffects", "GRM_Weather_Light", function()
        local ambient=GetGlobalFloat("GRM_AmbientFactor",GetGlobalFloat("GRM_SunFactor",1));local night=1-math.Clamp(ambient,0,1)
        local depth=GetGlobalFloat("GRM_NightDarkness",.5);local weather=GetGlobalString("GRM_Weather","clear")
        local targetExposure,targetColor=W.ExposureTarget(ambient,depth,outside,weather,cvNight:GetFloat())
        exposureNow=Lerp(math.Clamp(FrameTime()*.8,0,1),exposureNow,targetExposure);colorNow=Lerp(math.Clamp(FrameTime()*.6,0,1),colorNow,targetColor)
        DrawColorModify({
            ["$pp_colour_addr"]=night*.003,["$pp_colour_addg"]=night*.007,["$pp_colour_addb"]=night*.018,
            ["$pp_colour_brightness"]=-exposureNow,["$pp_colour_contrast"]=1+night*.045,
            ["$pp_colour_colour"]=math.Clamp(1-colorNow,.5,1),
            ["$pp_colour_mulr"]=0,["$pp_colour_mulg"]=0,["$pp_colour_mulb"]=night*.012,
        })
    end)
    ...
    net.Receive(OPEN, function()
        ...
        local nightDarkness=slider("Глубина ночного тона (не меняет освещение карты)",.2,.85,cfg.nightDarkness or .5,2)
