...
if SERVER then
    ...
    -- Код 112: обновлять состояние телефона при изменении инвентаря
    -- (иначе после покупки в /phoneshop телефон не появляется пока не пройдёт таймер)
    hook.Add("GRM_InventoryUpdated", "GRM_Mob_InvChange", function(ply)
        if IsValid(ply) then pushState(ply, true) end
    end)
