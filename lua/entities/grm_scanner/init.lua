...
-- вспомогательно: открыть/закрыть FFD-двери рядом
local function fadeDoorsNear(self, activate)
    local nearProps = ents.FindInSphere(self:GetPos(), DOOR_RADIUS)
    for _, prop in ipairs(nearProps) do
        if IsValid(prop) and prop.isFadingDoor then
            if activate and prop.FadeActivate then prop:FadeActivate()
            elseif not activate and prop.FadeDeactivate then prop:FadeDeactivate() end
        end
    end
    return nearProps
end
