...
function ENT:Draw()
    ...
    local pos=self:GetPos()+LABEL_OFFSET;local ang=Angle(0,EyeAngles().y-90,90);local fac=self:GetNWString("GRM_DutyFaction","");local configured=fac~=""and fac~="*"
    cam.Start3D2D(pos,ang,.075)
        ...
        draw.SimpleText(configured and fac or"НЕ НАСТРОЕН","GRMDutyNPC_Faction",0,-15,configured and C_OK or C_BAD,TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP)
