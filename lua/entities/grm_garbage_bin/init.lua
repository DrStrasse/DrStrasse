...
function ENT:Initialize()self:SetModel("models/props_junk/TrashDumpster02.mdl");self:PhysicsInit(SOLID_VPHYSICS);self:SetMoveType(MOVETYPE_VPHYSICS);self:SetSolid(SOLID_VPHYSICS);self:SetUseType(SIMPLE_USE);if self:GetBinName()==""then self:SetBinName("Мусорный контейнер")end;local ph=self:GetPhysicsObject();if IsValid(ph)then ph:Wake()end end
