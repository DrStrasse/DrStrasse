...
net.Receive("GRM_PressTerminal_Open", function()
    ...
    info.Paint = function(_, w, h)
        ...
        draw.SimpleText("Нагрев: " .. tostring(math.floor(heat)) .. "%", "GRMPressT_Normal", 14, 98, heat >= 80 and C.red or C.yellow, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText("Напечатано всего: " .. money(data.totalPrinted or 0), "GRMPressT_Small", 14, 122, C.dim, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
    ...
    hint:SetWrap(true)
    hint:SetText("Деньги печатаются в ГОСБЮДЖЕТ, паллеты дропаются в ближайшее банковское хранилище.")
end)
