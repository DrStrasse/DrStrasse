...
-- находка 123: без данных фракций окно всё равно собирается (DPanel) и
-- показывает ручное поле (DTextEntry), а не пропадает молча
local dpan1, dte1 = nil, nil
for _, c in ipairs(H.vguis or {}) do
  if c._cls == "DPanel" then dpan1 = c end
  if c._cls == "DTextEntry" then dte1 = c end
end
ok(dpan1 ~= nil and dte1 ~= nil, "нет данных фракций — окно (DPanel) с ручным DTextEntry на месте")
...
-- ════════════ ЧАСТЬ 3: Код 104/106 — интерактивное окно фракций ═════
print("== BuildCPanel: чекбоксы фракций из ЖИВОГО клиентского кэша ==")
...
ok(#H.textentries == 1 and H.textentries[1].convar == "ffd_keypad_password",
   "PIN-поле на месте, фракция — чекбоксами (не fallback-полем)")
