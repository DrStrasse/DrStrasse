...
ok(ES.MarkerHeight <= 48 and ES.MarkerHeight >= 16,
   "высота примерно на уровне глаз, не над крышей и не в полу", ES.MarkerHeight)
...
ok(src:find("local rot = CurTime()", 1, true) == nil,
   "убрано бессмысленное вращение вокруг оси — значок теперь читаемый")
ok(src:find("dir.z = 0", 1, true) ~= nil,
   "вертикальная составляющая направления обнуляется — значок не кувыркается")

-- Подпись должна следовать за опущенным значком.
ok(src:find("Vector(0, 0, 16)", 1, true) ~= nil,
   "подпись подтянута к новой высоте значка")
