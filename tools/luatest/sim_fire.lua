...
print("\n=== СЕРВЕР Код 58 ===")
check("версия 1.5.1", has(core, 'F.Version = "1.5.1"'))
...
local hydrantInit = read("addons/grm_fire/lua/entities/grm_fire_hydrant/init.lua")
local hydrantClient = read("addons/grm_fire/lua/entities/grm_fire_hydrant/cl_init.lua")
check("насос Consume agent", has(pump, "function ENT:Consume"))
check("новый насос стартует с полными баками", has(pump, "self:SetTank(WATER_MAX)") and has(pump, "self:SetFoam(FOAM_MAX)") and has(pump, "self:SetPowder(POWDER_MAX)"))
check("все изменения бака зовут SyncHost", has(pump, "function ENT:AgentAmount") and has(pump, "self:SyncHost()"))
check("закачка останавливается на полном баке", has(pump, "value >= (tonumber(maxv)") and has(pump, "self:SetFilling(false)"))
check("гидрант E переключает вентиль", has(hydrantInit, "local willOpen = not self:GetOpen()") and has(hydrantInit, "self:SetOpen(willOpen)"))
check("гидрант Shift+E выдаёт рукав отдельно", has(hydrantInit, "ply:KeyDown(IN_SPEED)") and has(hydrantInit, "A.TakeHose"))
check("подсказка гидранта актуальна", has(hydrantClient, "E закрыть") and has(hydrantClient, "Shift+E взять рукав"))
...
check("машина синхронизирует все ресурсы", has(pump, "GRM_FireFoamMax") and has(pump, "GRM_FirePowderMax") and has(pump, "GRM_FireHosesOut"))
check("truck HUD показывает current/max", has(truck, 'tank .. "/" .. tankMax') and has(truck, 'foam .. "/" .. foamMax') and has(truck, 'powder .. "/" .. powderMax'))
check("реагентный урон не затирается", not hose:find("local dmg = (GRM.FireAddon.HoseCfg", 1, true))
...
check("выключенный насос блокирует обход к гидранту", has(hoseInit, "Насос — граница графа") and has(hoseInit, "return false, ent"))
...
check("объём шланга +0.5", has(hoseLua, "HoseBeamHalfW = 1.85") and has(hoseLua, "HoseBeamHalfH = 0.38"))
check("нет второго opaque-хука рукава", not hoseLua:find('PostDrawOpaqueRenderables", "GRM_FireHose_Vis"', 1, true))
check("частая укладка LayStep 26", has(hoseLua, "LayStep     = 26") and has(hoseLua, "A.HoseCfg.LayStep = 26"))
...
check("TryLay досеивает несколько узлов", has(hoseInit, "local count = math.min(12") and has(hoseInit, "LerpVector(t, start, dest)"))
check("стыкованный путь сохраняет изгибы", has(hoseInit, "function ENT:TensionDockedPath") and not hoseInit:find("lays[k]:SetPos(self:GroundSnap(LerpVector", 1, true))
check("клиент сглаживает ломаную", has(hoseLua, "function A.SmoothHosePath") and has(hoseLua, "LerpVector(0.25"))
check("стационарная линия имеет S-слабину", has(hoseLua, "HoseSupplySlack") and has(hoseLua, "local wave = math.sin"))
...
check("лимит стационарной линии", has(hoseInit, "function ENT:CheckOverstretch"))
check("автосмотка перерастянутого рукава", has(hoseInit, "GRM_FireAddon_HoseOverstretched") and has(hoseInit, "self:Rewind()"))
check("grace рукава", has(hoseLua, "BreakGrace") and has(hoseLua, "OverstretchSec"))
...
check("гидрант пожаробезопасен", has(core, "grm_fire_hydrant = true") and has(core, "GRM_Fire_Fireproof"))
check("Ignite отвергает пожарное оборудование", has(core, "пожарное оборудование не может быть источником огня"))
...
check("статус v1.5.0", has(status, "StatusVersion = \"1.5.0\""))
...
check("не синтезирует локализован при полном тушении", not status:find("F.MarkLocalized(inc)\n        end\n        inc.out", 1, true))
check("NoteFight не создаёт пожар от одной струи", has(status, "Никогда не открываем инцидент просто от струи"))
check("одно уведомление без обязательного дубля", not has(status, "всегда дубль в чат"))
check("граждане рядом не получают статусный спам", not has(status, "1500 * 1500"))
