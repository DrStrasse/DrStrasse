...
ok(weather:find("engine.LightStyle",1,true)and weather:find("nightStyle",1,true)and weather:find('nightDarkness=.4',1,true),"midnight lighting is dim but readable by default")
...
ok(weather:find("GRM_NightDarkness",1,true)and weather:find("brightnessLoss",1,true)and weather:find("Темнота ночи",1,true),"night darkness is balanced and configurable")
...
local rainFile=assert(io.open("sound/grm/weather/rain_loop.wav","rb"));local rainSize=rainFile:seek("end");rainFile:close()
local stormFile=assert(io.open("sound/grm/weather/storm_loop.wav","rb"));local stormSize=stormFile:seek("end");stormFile:close()
...
ok(rainSize>700000 and stormSize>700000 and weather:find('W.Version="1.1.2"',1,true),"detailed 24-second water/drop layers replace television noise")
...
ok(weather:find("ambient_noir.wav",1,true)and weather:find('musicTheme="noir"',1,true)and weather:find("Нуарный детектив",1,true),"slow noir detective ambience is the default theme")
ok(weather:find("resource.AddFile",1,true)and weather:find('file.Exists(path, "GAME")',1,true),"custom audio is downloaded and checked before BASS playback")
ok(weather:find("retryAt",1,true)and weather:find("grm_weather_audio_debug",1,true),"missing BASS files do not spam and expose diagnostics")
