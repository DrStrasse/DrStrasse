...
local netsent = {}
local curFrame = nil
net = { Start = function(m) curFrame = { msg = m, tableV = nil, uintV = nil } end,
...
ok(mylist ~= nil and mylist.tableV[1].locked == true, "«Мои Т/С»: locked=true (Диллер 2.1)")
ok(mylist ~= nil and type(mylist.tableV[1].dist) == "number", "«Мои Т/С»: dist числом (Диллер 2.1)")
...
ok(ctxRes ~= nil and ctxRes.tableV.veh.refund == 200, "ctx: refund=200 — подпись кнопки «вернуть…» (2.1)")
...
-- ═══ ДИЛЛЕР 2.1 ═══
-- 10) E на дилере → VD_OpenMenu: у позиций есть модель превью, 4-е поле = баланс
Pete.pos = dealer.pos
if ENT and ENT.Use then ENT.Use(dealer, Pete, Pete) end
local om = lastNet("VD_OpenMenu")
ok(om ~= nil and istable(om.tableV) and #om.tableV >= 1, "VD_OpenMenu по E: список выдан")
ok(om ~= nil and om.tableV[1].model == "models/vehicle.mdl",
    "VD_OpenMenu: модель превью резолвнута из list.Vehicles: " .. tostring(om and om.tableV[1].model))
ok(om ~= nil and om.uintV == GRM.GetBalance(Pete),
    "VD_OpenMenu: 4-е поле — баланс наличных (" .. tostring(om and om.uintV) .. ")")
ok(H.netrecv["VD_MyListReq"] ~= nil, "VD_MyListReq receiver жив (2.1)")
local netsN = #netsent
if H.netrecv["VD_MyListReq"] then H.netrecv["VD_MyListReq"](0, Pete) end
ok(#netsent > netsN and lastNet("VD_MyList") ~= nil, "VD_MyListReq → свежий VD_MyList отправлен")

-- 11) Гараж: сдаём далёкую машину через fromMenu, стоя у дилера
-- (ctx-блок выше переопределял net.ReadEntity на nil — возвращаем pop-вариант)
net.ReadEntity = function() local v = table.remove(seqVals, 1) return v end
GRM.SetBalance(Pete, 1000)
seqVals = { "d1", "jalopy" }
if H.netrecv["VD_SpawnRequest"] then H.netrecv["VD_SpawnRequest"](0, Pete) end
local vd4 = nil
for id, e in pairs(VD_AllVehicles) do if IsValid(e) and e.VD_Owner == Pete then vd4 = e end end
ok(vd4 ~= nil, "гараж: машина заспавнена")
vd4.pos = Vector(5000, 0, 0) -- машина далеко
seqVals = { vd4, true }      -- entity + fromMenu=true
if H.netrecv["VD_RemoveRequest"] then H.netrecv["VD_RemoveRequest"](0, Pete) end
ok(vd4.removed == true, "гараж: далёкая машина сдана удалённо (Пётр у дилера)")
ok(GRM.GetBalance(Pete) == 800, "гараж: возврат 200 начислен независимо от дистанции до Т/С")

-- 12) fromMenu вдали от любого дилера отклоняется; обычная сдача рядом работает
seqVals = { "d1", "jalopy" }
if H.netrecv["VD_SpawnRequest"] then H.netrecv["VD_SpawnRequest"](0, Pete) end
local vd5 = nil
for id, e in pairs(VD_AllVehicles) do if IsValid(e) and e.VD_Owner == Pete then vd5 = e end end
ok(vd5 ~= nil, "машина для анти-абьюз теста заспавнена")
Pete.pos = Vector(9000, 0, 0) -- Пётр далеко от дилеров
seqVals = { vd5, true }
if H.netrecv["VD_RemoveRequest"] then H.netrecv["VD_RemoveRequest"](0, Pete) end
ok(vd5.removed ~= true, "гараж: вдали от дилеров удалённая сдача отклонена")
Pete.pos = Vector(0, 0, 0)
vd5.pos = Vector(100, 0, 0)
seqVals = { vd5 } -- fromMenu=false: классическая сдача рядом (≤600)
if H.netrecv["VD_RemoveRequest"] then H.netrecv["VD_RemoveRequest"](0, Pete) end
ok(vd5.removed == true, "обычная сдача рядом — как прежде (600 → возврат 200)")
ok(GRM.GetBalance(Pete) == 600, "баланс согласован: 800 − 400 + 200 = 600")
