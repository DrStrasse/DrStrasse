...
**Самая стабильная версия:** `2a351e6` (`STABLE.md`, подтверждена владельцем 2026-08-15).
...
Сначала читать `STABLE.md`, затем актуальный `HANDOVER.md` и находки **129**, **134**, **135**, **136**, **137**, **138**, **139** в `ANALYSIS.md`.
...
- RoomTap 1.1: permanent + temporary + access; Alarm/CCTV heal существующее по DeviceID и валидируют JSON.
- Electronics SaveAll всегда пытается записать map и DB, LoadAll не трогает карту при битой DB.
- Guard/Hub 1.2 покрывают supporting JSON, cleanup и watchdog.
- Контроль: **491/491 syntax, 79/79 sim, 16/16 roundtrip**.

## Предыдущий фикс: единое меню сохранений (находка 135)

- «Телефония» использует полный контракт, не удаляет купленные телефоны и проверяет JSON до замены карты.
- «Еда и кухня» сохраняет vending + stove/fridge/planter с текущим содержимым через Perm 1.7.1.
- «Завод» сохраняет карту/склады + шкафы + скупщиков + рынок; supporting JSON получили backup/read-back/fail-closed.
- Незагруженный модуль больше не показывается как успешная операция.
- Контроль после этого пакета: **491/491 syntax, 79/79 sim, 16/16 roundtrip**.

## Предыдущий фикс: Perm-двери и близкие пропы (находка 134)
...
- Контроль: **491/491 syntax, 79/79 sim, 16/16 roundtrip**; после любых правок повторить полный чеклист.
