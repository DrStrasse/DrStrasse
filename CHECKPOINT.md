...
> ghost-превью, Derma-скины, Markdown-рендер). Ключевые выводы: (1) для
> «компьютера со своей ОС» — схема GTerminal (OS-модули + per-entity ФС в памяти +
> пароль + net-консоль); (2) для документов/книг — Markdown-парсер HLX_Books;
> (3) для лицензий — Helix-паттерн «предмет-пермит гейтит покупку» поверх нашего
> `DOC.HasValid*License`; (4) для UI — Ciero-скин и MapStudio-виджеты; (5) для
> порталов/шлюзов — stencil+ClientsideModel. Архивы в git **не распаковывались**.
