...
**HEAD (local = remote):** `4962c88` `docs(fire): чекпоинт…` поверх `3430a00` (учёт тушения)  
...
1. `git fetch origin arena/019ffaa2-drstrasse && git reset --mixed FETCH_HEAD && git log -1 --oneline` — убедиться что HEAD `4962c88` или новее (`3430a00` = код учёта без этого файла).
